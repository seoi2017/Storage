...
struct segtree{
    ...
    void pushdown(integer x){
        ...
        //===============
        node[x]*=lazy_mul[x];
        lazy_mul[x<<1]+=lazy_mul[x],lazy_mul[(x<<1)+1]+=lazy_mul[x];
        lazy_mul[x]=0;
    }
    ...
    void update_mul(integer x,integer l,integer r,integer s){
        ...
        pushdown(x);
        node[x]*=s;
        if(r<=mid[x])update_mul(x<<1,l,r,s);
        else if(l>mid[x])update_mul((x<<1)+1,l,r,s);
        else update_mul(x<<1,l,mid[x],s),update_mul((x<<1)+1,mid[x]+1,r,s);
    }
...
int main(){
    ...
    while(ask--){
        ...
        scanf("%d",&command);
        if(command==1)scanf("%d%d%d",&x,&y,&o),tree.update_mul(1,tree.get_node(x),tree.get_node(y),o);
