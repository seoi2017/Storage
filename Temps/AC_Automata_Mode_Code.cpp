...
int top = 0, tot = 0, vis[MAX_CHARS * MAX_LENGTH]; //top是节点数量，tot是模式串数量，vis是节点是否已访问数组
char modes[MAX_STRINGS + 1][MAX_LENGTH];           //模式串存储数组
void insert_trie(char str[], ACNode *root, int number)
{                     //将模式串插入trie树，number是模式串编号
    ACNode *k = root; //迭代指针，指向根节点
    int s = 0, index; //s是字符位数指针，index是字符集下标指针
    while (str[s])
    {                         //如果有字符
        index = str[s] - 'a'; //取整形字符下标
        if (k->next[index] == NULL)
        {                                       //如果当前节点子节点中无想要插入的节点
...
void build_AC_Automata(ACNode *root)
{                      //构建AC自动机
        ...
        { //依照字符集遍历子节点
            ...
            { //如果该字符子节点不为空
                ...
                {                          //如果当前节点不是根节点
                    ...
                    while (fail_ptr != NULL)
                    { //如果临时失配指针不为空
                        if (fail_ptr->next[i] != NULL)
                        {                                            //如果临时失配指针指向节点的相同字符子节点不为空
                    ...
                    if (fail_ptr == NULL)
                    {                               //如果临时失配指针为空，则找到了根节点，因为只有根节点的失配指针为空
